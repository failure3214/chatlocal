...
def start_auth_window():
    ...
    def login_thread():
        ...
        if response.startswith("OK|"):
            ...
            if len(parts) == 2:
                user_id = int(parts[1])
                root.after(0, lambda: start_chat(username, user_id, ip))
                root.destroy()
            else:
...
ip_window = tk.Tk()
ip_window.title("Enter Server IP")
ip_window.geometry("300x120")
ip_window.configure(bg="#2f2f2f")
tk.Label(ip_window, text="Server IP:", bg="#2f2f2f", fg="white", font=("Arial", 12)).pack(pady=(20,5))
ip_entry = tk.Entry(ip_window, font=("Arial", 12))
ip_entry.pack(pady=5)
tk.Button(ip_window, text="Set IP", command=set_ip, bg="#4a4a4a", fg="white", font=("Arial", 12)).pack(pady=10)
ip_window.mainloop()
